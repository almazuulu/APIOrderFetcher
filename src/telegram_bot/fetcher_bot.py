...
async def send_welcome(message: Message):
    await message.answer(
        "Привет! Используй команду /getfile, чтобы получить последние файлы Excel.",
    )
...
async def send_excel_files(message: Message):
    ...
    if order_files:
        latest_order_file = max(
            order_files,
            key=lambda x: os.path.getctime(os.path.join(EXCEL_FOLDER_PATH, x)),
        )
        order_file_path = os.path.join(EXCEL_FOLDER_PATH, latest_order_file)
        order_file = FSInputFile(order_file_path)
        await message.answer_document(
            document=order_file,
            caption="Вот последний файл заказов:",
        )

    if sale_files:
        latest_sale_file = max(
            sale_files,
            key=lambda x: os.path.getctime(os.path.join(EXCEL_FOLDER_PATH, x)),
        )
        ...
        await message.answer_document(
            document=sale_file,
            caption="Вот последний файл продаж:",
        )
